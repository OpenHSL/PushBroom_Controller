...
- number_of_steps количество шагов для шагового механизма (задаётся целым числом);
- exposure выдержка в мс (подбирается экспериментально, оценивая освещенность сцены; задаётся целым числом, можно начинать с 100 000);
- gain коэффициент усиления сигнала (подбирается экспериментально, оценивая освещенность сцены; задаётся целым числом больше 0);
